...
int aerial_phy_nr_crc_indication(nfapi_nr_crc_indication_t *ind) {
  ...
  LOG_I(NR_MAC,"sfn %d, slot %d, numCRC %d, Handle %d, RNTI, %d, HARQID %d");
...
int aerial_phy_nr_rach_indication(nfapi_nr_rach_indication_t *ind)
{
  if(NFAPI_MODE == NFAPI_MODE_AERIAL)
  {
    ...
    {
      printf("rach_ind = %p\n",ind);
      printf("rach_ind->pdu_list[i] = %p\n",&ind->pdu_list[i]);
      printf("rach_ind->pdu_list[i].num_preamble = %d\n",ind->pdu_list[i].num_preamble);
      printf("ind = %p\n",ind);
      printf("ind->pdu_list[i] = %p\n",&ind->pdu_list[i]);
      printf("ind->pdu_list[i].num_preamble = %d\n",ind->pdu_list[i].num_preamble);
...
int fapi_nr_p7_message_pack(void *pMessageBuf, void *pPackedBuf, uint32_t packedBufLen, nfapi_p7_codec_config_t* config)
{
  ...
  switch (pMessageHeader->message_id)
  {
    ...
    case NFAPI_NR_PHY_MSG_TYPE_UL_DCI_REQUEST:
      printf("asdfasdfasdfasdfasdfasdf UL_DCI_REQUEST 0x83\n\n\n");
      return 0;
...
int fapi_nr_pack_and_send_p7_message(vnf_p7_t* vnf_p7, nfapi_p7_message_header_t* header){
  ...
  if(header->message_id == NFAPI_NR_PHY_MSG_TYPE_UL_DCI_REQUEST){
    return 0;
  }
