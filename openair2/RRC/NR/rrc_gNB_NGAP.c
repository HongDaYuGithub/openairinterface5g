...
//------------------------------------------------------------------------------
{
  ...
  for (int i = 0; i < NR_NB_RB_MAX - 3; i++) {
    if(ue_context_p->ue_context.pduSession[i].status >= PDU_SESSION_STATUS_DONE)
      continue;
    ue_context_p->ue_context.pduSession[i].status      = PDU_SESSION_STATUS_NEW;
    ue_context_p->ue_context.pduSession[i].param       = msg->pdusession_setup_params[pdu_sessions_done];
    create_tunnel_req.pdusession_id[pdu_sessions_done] = msg->pdusession_setup_params[pdu_sessions_done].pdusession_id;
    create_tunnel_req.incoming_rb_id[pdu_sessions_done]= i+1;
    create_tunnel_req.outgoing_teid[pdu_sessions_done]  = msg->pdusession_setup_params[pdu_sessions_done].gtp_teid;
    memcpy(create_tunnel_req.dst_addr[pdu_sessions_done].buffer,
           msg->pdusession_setup_params[pdu_sessions_done].upf_addr.buffer,
           sizeof(uint8_t)*20);
    create_tunnel_req.dst_addr[pdu_sessions_done].length = msg->pdusession_setup_params[pdu_sessions_done].upf_addr.length;
